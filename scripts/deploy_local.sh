...
FORCE_RESET="${FORCE_RESET:-0}"    # 1=kill existing anvil on $PORT first
...
start_anvil() {
  # Optional hard reset on port
  if [[ "$FORCE_RESET" = "1" ]]; then
    echo "[deploy] FORCE_RESET=1 → killing any process on :$PORT"
...
cleanup() {
  # Optionally stop anvil started by this script
...
if [[ "$USE_MNEMONIC" = "1" ]]; then
  echo "[deploy] Using MNEMONICS for deploy ($DERIVATION_PATH_TX)"
...
# Write frontend env (no dev server here)
...
# Optional: stream Anvil logs while KEEP_ANVIL=1
if [[ "$KEEP_ANVIL" = "1" && -n "${ANVIL_PID:-}" ]]; then
  echo "[deploy] Tailing Anvil logs (Ctrl+C to stop tail; Anvil keeps running)..."
  tail --pid="${ANVIL_PID}" -f "$logfile"
fi
