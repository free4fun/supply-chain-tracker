        ...
        <ul className="grid gap-2">
          {items.map(it => {
            if (it.type === "transfer") {
              ...
              return (
                <li key={`tr-${it.id}`} className="rounded-xl border border-surface bg-surface-2 p-3 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-semibold">#{it.tokenId} · {it.tokenName ?? `Token ${it.tokenId}`} · {it.amount.toString()}</p>
              ...
              // creation
              return (
                <li key={`cr-${it.id}`} className="rounded-xl border border-surface bg-surface-2 p-3 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-semibold">#{it.tokenId} · {it.tokenName ?? `Token ${it.tokenId}`} · {it.totalSupply.toString()}</p>
